...
Replace `@bull-board/elysia` with `@getworkbench/elysia` — same Redis, same queues, no schema change. Remove the old bull-board mount, then run `npx @getworkbench/cli init` or follow the install steps below.
