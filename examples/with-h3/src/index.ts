...
const workbenchHandler = workbench({
  queues,
  title: "h3 · Workbench",
  basePath: "/jobs",
  auth,
...
const router = createRouter()
  .get(
    "/",
    defineEventHandler(() => "Try /jobs for the Workbench dashboard.\n"),
  )
  // h3's `**` only matches one-or-more sub-segments, so register at both
  // the bare prefix and the catch-all.
  .use("/jobs", workbenchHandler)
  .use("/jobs/**", workbenchHandler);
